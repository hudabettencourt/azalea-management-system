...
export default function VarianBoronganTab({ C, isDark, showToast }: Props) {
  ...
  const fetchVarian = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from("varian_borongan").select("*").order("nama");
    ...
    <div style={{ animation: "fadeUp 0.3s ease" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 12, color: C.muted, fontFamily: C.fontMono }}>
            Master varian & tarif upah borongan pencetak siomay
          </p>
        </div>
        <button onClick={() => { setShowTambah(v => !v); setTambahNama(""); setTambahTarif(""); }}
        ...
        <div style={{ background: `${C.accent}08`, border: `1px solid ${C.accent}30`, borderRadius: 14, padding: 24, marginBottom: 20, animation: "slideDown 0.2s ease" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 12, marginBottom: 14 }}>
            <div>
              ...
              <input value={tambahNama} onChange={e => setTambahNama(e.target.value)} placeholder="Contoh: Kuncup, Besar" style={inputStyle} autoFocus onKeyDown={e => e.key === "Enter" && handleTambah()} />
            </div>
            <div>
              <div style={{ fontSize: 10, color: C.muted, fontFamily: C.fontMono, letterSpacing: 1, marginBottom: 5, textTransform: "uppercase" as const }}>TARIF / KG (Rp) *</div>
              <input value={tambahTarif} onChange={e => setTambahTarif(formatIDR(e.target.value))} placeholder="0" style={{ ...inputStyle, fontFamily: C.fontMono, fontWeight: 700 }} onKeyDown={e => e.key === "Enter" && handleTambah()} />
            </div>
          </div>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 100px 140px", gap: 8, padding: "10px 24px", borderBottom: `1px solid ${C.border}`, background: isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)" }}>
          {["NAMA VARIAN", "TARIF / KG", "STATUS", "AKSI"].map(h => (
        ...
        {!loading && varianList.length === 0 && (
          <div style={{ padding: 40, textAlign: "center", color: C.muted, fontFamily: C.fontMono }}>Belum ada varian — tambah dulu!</div>
        )}

        {!loading && varianList.map(v => (
          <div key={v.id} style={{ borderBottom: `1px solid ${C.border}`, background: editingId === v.id ? `${C.accent}06` : "transparent" }}>
            {editingId !== v.id ? (
              <>
                <div className="data-row" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 100px 140px", gap: 8, padding: "14px 24px", alignItems: "center", transition: "background 0.15s" }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{v.nama}</div>
                  <div style={{ fontSize: 14, fontWeight: 800, color: C.green, fontFamily: C.fontMono }}>{rupiahFmt(v.tarif_per_kg)}<span style={{ fontSize: 11, fontWeight: 400, color: C.muted }}>/kg</span></div>
                  <div>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {confirmHapusId === v.id ? (
                      ...
                      <>
                        <button className="btn-edit" onClick={() => { setEditingId(v.id); setEditNama(v.nama); setEditTarif(formatIDR(String(v.tarif_per_kg))); }}
                        ...
                        <button className="btn-del" onClick={() => setConfirmHapusId(v.id)}
              ...
              <div style={{ padding: "14px 24px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 12, marginBottom: 12 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 10, color: C.muted, fontFamily: C.fontMono, letterSpacing: 1, marginBottom: 5, textTransform: "uppercase" as const }}>TARIF / KG</div>
