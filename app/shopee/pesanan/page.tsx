...
// Tugas 2: pill filters (toko/jasa-kirim/status with counts), search,
// product-image column (📦 fallback), batas-waktu countdown,
// per-row + bulk Atur Pengiriman + Print Label, Scan Resi entry.
...
function formatBatasWaktu(tanggalPesanan: string): { text: string; danger: boolean; expired: boolean } {
  ...
  if (hours < 24) {
    return { text: `${hours}j tersisa`, danger: true, expired: false };
  }
...
// Decode base64 PDF blob and open in a new tab. Used by single + bulk print.
function openBase64Pdf(b64: string, filename: string) {
  try {
    ...
    // Some browsers block popups; fall back to a download link.
    if (!w) {
    ...
    console.error("openBase64Pdf failed:", err);
    alert("Gagal membuka PDF. Cek console untuk detail.");
  }
}

// Defensive extractor — Shopee's download_shipping_document response shape
// varies by endpoint version. Walk common shapes; return null if nothing
// PDF-looking found.
function extractPdfBase64(raw: any): string | null {
  if (!raw) return null;
  const candidates = [
    raw?.response?.result,
    raw?.response?.shipping_document_info?.[0]?.shipping_document,
    raw?.response?.shipping_document,
    raw?.response?.data,
  ];
...
function PillFilterRow({
  label, options, selected, onSelect, C,
}: {
  label: string;
        ...
        {options.map(opt => {
          ...
          return (
            <button key={opt.key} onClick={() => onSelect(opt.key)} style={{
              ...
              {opt.label}
              {opt.count !== undefined && (
                <span style={{ marginLeft: 6, opacity: 0.7, fontFamily: C.fontMono, fontSize: 11 }}>
                  ({opt.count})
                </span>
              )}
            </button>
...
function ShipModal({
  open, onClose, targets, onShipped, C,
}: {
  open: boolean;
  onClose: () => void;
  targets: ShipTarget[];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Use first target as representative for param shape (bulk is
        // restricted to a single toko by the caller).
        const t = targets[0];
        const res = await fetch(`/api/shopee/get-shipping-parameter?toko_id=${t.toko_id}&order_sn=${encodeURIComponent(t.order_sn)}`);
        const data = await res.json();
        if (cancelled) return;
        if (data.error || data.raw?.error) {
          setParamsErr(data.error || data.raw?.message || data.raw?.error);
        } else {
          setParamsRaw(data.raw?.response ?? data.raw);
        }
      } catch (err: any) {
        if (!cancelled) setParamsErr(err.message);
      } finally {
  ...
  // Best-effort parse of pickup time slots from the params response.
  const pickupSlots: ShipParamPickupSlot[] = useMemo(() => {
    ...
    for (const addr of addrList) {
      const tList = addr?.time_slot_list || [];
      for (const s of tList) slots.push(s);
    }
    return slots;
  }, [paramsRaw]);
  ...
  const submit = async () => {
    ...
    setSubmitErr(null);
    try {
      // All targets share the same toko (enforced upstream).
      ...
      const res = await fetch("/api/shopee/ship-order", {
        method: "POST",
      ...
      const data = await res.json();
      if (!data.success) {
        setSubmitErr(data.error || "Gagal mengatur pengiriman");
        return;
      }
      onShipped();
      onClose();
  ...
  return (
    ...
    >
      <div onClick={e => e.stopPropagation()} style={{
        ...
        maxWidth: 520, width: "100%", padding: "20px 24px", boxShadow: C.shadowMd,
        maxHeight: "90vh", overflowY: "auto",
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: C.text, fontFamily: C.fontSans }}>
            {isBulk ? `Atur Pengiriman (${targets.length} pesanan)` : "Atur Pengiriman"}
          </div>
        ...
        <div style={{ fontSize: 11, color: C.muted, fontFamily: C.fontMono, marginBottom: 14 }}>
          {isBulk
            ? `${targets[0].toko_nama} · ${targets.map(t => t.order_sn).slice(0, 3).join(", ")}${targets.length > 3 ? "…" : ""}`
            : `${targets[0]?.toko_nama || ""} · ${targets[0]?.order_sn || ""}`}
        </div>

        {loadingParams && (
          <div style={{ color: C.muted, fontFamily: C.fontMono, fontSize: 13, padding: "20px 0" }}>
            Memuat parameter pengiriman...
          </div>
        )}

        {paramsErr && (
          <div style={{ padding: 12, background: C.redDim, color: C.red, borderRadius: 10, fontSize: 12, marginBottom: 14 }}>
            ⚠ {paramsErr}
          </div>
        )}
          ...
          <>
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              {(["pickup", "dropoff"] as const).map(m => (
                <button key={m} onClick={() => setMethod(m)} style={{
                  ...
                  background: method === m ? `${C.accent}20` : "transparent",
                  border: `1.5px solid ${method === m ? C.accent : C.border}`,
                  color: method === m ? C.accent : C.muted,
                  cursor: "pointer", fontWeight: 700, fontSize: 13,
                  fontFamily: C.fontSans, textTransform: "capitalize",
                }}>
            ...
            {method === "pickup" && (
              <div>
                ...
                {pickupSlots.length === 0 ? (
                  <div style={{ fontSize: 12, color: C.muted, fontFamily: C.fontMono, padding: 8 }}>
                    Tidak ada time slot tersedia (kosong di response Shopee).
                  </div>
                ) : (
                  <div style={{ maxHeight: 220, overflowY: "auto", border: `1px solid ${C.border}`, borderRadius: 10 }}>
                    {pickupSlots.map((s, i) => {
                      ...
                      const dateLabel = typeof s.date === "number"
                        ? new Date(s.date * 1000).toLocaleDateString("id-ID", { day: "2-digit", month: "short" })
                        : s.date || "";
                      return (
                        <button key={id} onClick={() => setPickupSlot(id)} style={{
                          display: "block", width: "100%", textAlign: "left",
                          padding: "10px 12px", border: "none",
                          background: active ? `${C.accent}20` : "transparent",
                          color: active ? C.accent : C.text, fontWeight: active ? 700 : 500,
                          borderBottom: `1px solid ${C.border}`, cursor: "pointer",
                          fontSize: 12, fontFamily: C.fontSans,
                        }}>
                          {dateLabel} · {s.time_text || "(no label)"}
                        </button>
                      );
                    })}
                  </div>
            ...
            {method === "dropoff" && (
              <div>
                ...
                {branches.length === 0 ? (
                  <div style={{ fontSize: 12, color: C.muted, fontFamily: C.fontMono, padding: 8 }}>
                    Tidak ada cabang dropoff tersedia. Lanjut tanpa branch_id juga bisa (Shopee akan pilih default).
                  </div>
                ) : (
                  <div style={{ maxHeight: 220, overflowY: "auto", border: `1px solid ${C.border}`, borderRadius: 10 }}>
                    {branches.map((b, i) => {
                      ...
                      return (
                        <button key={id} onClick={() => setBranchId(id)} style={{
                          display: "block", width: "100%", textAlign: "left",
                          padding: "10px 12px", border: "none",
                          background: active ? `${C.accent}20` : "transparent",
                          color: active ? C.accent : C.text, fontWeight: active ? 700 : 500,
                          ...
                          <div>{b.region || "(no region)"}</div>
                          <div style={{ fontSize: 10, color: C.muted, fontFamily: C.fontMono, marginTop: 2 }}>{b.address || ""}</div>
                        </button>
                      );
                    })}
                  </div>
            ...
            {submitErr && (
              <div style={{ marginTop: 12, padding: 10, background: C.redDim, color: C.red, borderRadius: 8, fontSize: 12 }}>
                {submitErr}
              </div>
            )}
...
export default function OrdersPage() {
  ...
  // Apply filters in stages so we can compute counts for each dimension
  // ignoring its own filter (so toggling Toko=AzaleaFood doesn't dim every
  // other status count to 0).
  ...
  const filterExcept = useCallback((skip: "toko" | "jasa" | "status") => {
  ...
  const tokoCounts = useMemo(() => {
    const data = filterExcept("toko");
    const map = new Map<string, number>();
    map.set("semua", data.length);
    for (const o of data) map.set(String(o.toko_id), (map.get(String(o.toko_id)) || 0) + 1);
    return map;
  }, [filterExcept]);

  const jasaCounts = useMemo(() => {
    const data = filterExcept("jasa");
    const map = new Map<string, number>();
    map.set("semua", data.length);
    for (const o of data) {
      const k = o.jasa_kirim || "";
      if (!k) continue;
  ...
  const statusCounts = useMemo(() => {
    const data = filterExcept("status");
    const map = new Map<string, number>();
    map.set("semua", data.length);
    for (const o of data) {
      const k = o.status_shopee || "";
      if (!k) continue;
      map.set(k, (map.get(k) || 0) + 1);
    }
    return map;
  ...
  const filtered = useMemo(() => filterExcept("none" as any), [filterExcept]);
  ...
  const handleShipSingle = (o: Order) => {
    setShipModalTargets([{ toko_id: o.toko_id, toko_nama: o.nama_toko, order_sn: o.no_pesanan }]);
  };

  const handleShipBulk = () => {
    if (selectedShippable.length === 0) {
      showToast("Pilih pesanan dengan status Perlu Dikirim / Diproses", "error");
      return;
    }
    // Restrict bulk to a single toko — different tokos use different tokens
    // and shipping options, so a one-modal-fits-all flow doesn't apply.
    const tokoIds = new Set(selectedShippable.map(o => o.toko_id));
    if (tokoIds.size > 1) {
      showToast("Pilih pesanan dari 1 toko saja untuk Atur Pengiriman Massal", "error");
      return;
    }
  ...
  const printOneToko = async (tokoId: number, orders: Order[]) => {
    ...
    if (!b64) {
      console.error("[print-label] response did not contain base64 PDF:", data.raw);
      throw new Error("PDF tidak ditemukan di response (cek server log)");
    }
  ...
  const handlePrintSingle = async (o: Order) => {
    setPrinting(true);
    try {
      await printOneToko(o.toko_id, [o]);
    } catch (err: any) {
  ...
  const handlePrintBulk = async () => {
    ...
    setPrinting(true);
    try {
      // Group by toko_id so each call uses the right toko's auth.
      const byToko = new Map<number, Order[]>();
      for (const o of target) {
        const arr = byToko.get(o.toko_id) || [];
        arr.push(o);
        byToko.set(o.toko_id, arr);
      }
      for (const [tokoId, orders] of byToko) {
        await printOneToko(tokoId, orders);
      }
  ...
  const onShipped = () => {
    showToast("✓ Pengiriman berhasil diatur");
    setShipModalTargets([]);
    fetchAll();
  };
  ...
  const tokoOptions = [
    { key: "semua", label: "Semua", count: tokoCounts.get("semua") || 0 },
    ...tokoList.map(t => ({ key: String(t.id), label: t.nama, count: tokoCounts.get(String(t.id)) || 0 })),
  ];
  const jasaOptions = [
    { key: "semua", label: "Semua", count: jasaCounts.get("semua") || 0 },
    ...jasaKirimList.map(j => ({ key: j, label: j, count: jasaCounts.get(j) || 0 })),
  ];
    ...
    <Sidebar pageTitle="Shopee · Pesanan" pageSubtitle="Order management + logistics">
      ...
      {toast && (
        <div style={{
          ...
          fontSize: 13, fontWeight: 700, boxShadow: C.shadowMd, animation: "fadeUp 0.2s ease",
        }}>{toast.msg}</div>
      )}

      <ShipModal
        open={shipModalTargets.length > 0}
        onClose={() => setShipModalTargets([])}
        targets={shipModalTargets}
        onShipped={onShipped}
        C={C}
      />
      ...
      <div style={{ padding: "24px 28px", animation: "fadeUp 0.3s ease" }}>
        ...
        <div className="no-print" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <Link href="/shopee/pesanan/scan-resi" style={{
              ...inputStyle, textDecoration: "none", cursor: "pointer",
            ...
            <button onClick={handleSyncAll} disabled={syncing} style={{
              ...
              fontSize: 13, fontWeight: 700, fontFamily: C.fontSans, opacity: syncing ? 0.7 : 1,
        ...
        {selected.size > 0 && (
          <div className="no-print" style={{
            ...
            <div style={{ fontSize: 12, fontWeight: 700, color: C.accent, fontFamily: C.fontSans, flex: 1 }}>
              {selected.size} pesanan dipilih · {selectedShippable.length} bisa dikirim
            </div>
            <button onClick={handleShipBulk} disabled={selectedShippable.length === 0} style={{
              ...inputStyle, cursor: "pointer", fontWeight: 700,
              opacity: selectedShippable.length === 0 ? 0.5 : 1,
            }}>🚚 Atur Pengiriman Massal</button>
            <button onClick={handlePrintBulk} disabled={printing || selectedShippable.length === 0} style={{
              ...inputStyle, cursor: "pointer", fontWeight: 700,
              opacity: printing || selectedShippable.length === 0 ? 0.5 : 1,
            }}>🖨 Print Label Massal</button>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden", boxShadow: C.shadow }}>
          ...
          <div style={{
            display: "grid",
            gridTemplateColumns: "32px 52px 1fr 110px 100px 110px 110px 110px 220px",
            ...
            <span>Foto</span>
            <span>Produk / Pesanan / Pembeli</span>
              ...
              <div key={order.id} className="order-row" style={{
                ...
                <input type="checkbox" checked={checked} onChange={() => {
                  setSelected(prev => {
                    ...
                    if (n.has(order.id)) n.delete(order.id); else n.add(order.id);
                    return n;
                  });
                }} style={{ cursor: "pointer" }} />

                {/* Foto produk — placeholder until stok_barang.foto_url is added */}
                <div style={{
                ...
                {/* Produk / pesanan / pembeli */}
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: C.text, fontFamily: C.fontSans, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {order.nama_produk} <span style={{ color: C.muted, fontWeight: 500, fontSize: 12 }}>×{order.qty}</span>
                  </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{
                    fontSize: 10, padding: "3px 8px", borderRadius: 12, alignSelf: "flex-start",
                    background: sc?.bg || C.border, color: sc?.color || C.muted,
                    fontWeight: 700, fontFamily: C.fontMono,
                  }}>{order.status_shopee || "—"}</span>
                  {isShippable && (
                    <div style={{ display: "flex", gap: 4 }}>
                      <button onClick={() => handleShipSingle(order)} style={{
                        padding: "4px 8px", borderRadius: 6, fontSize: 10, fontWeight: 700,
                        background: `${C.accent}20`, color: C.accent,
                        border: `1px solid ${C.accent}`, cursor: "pointer", fontFamily: C.fontSans,
                      }}>Atur Pengiriman</button>
                      <button onClick={() => handlePrintSingle(order)} disabled={printing} style={{
                        padding: "4px 8px", borderRadius: 6, fontSize: 10, fontWeight: 700,
                        background: "transparent", color: C.muted,
                        border: `1px solid ${C.border}`, cursor: "pointer", fontFamily: C.fontSans,
