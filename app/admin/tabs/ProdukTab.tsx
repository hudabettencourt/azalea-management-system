...
const emptyForm = (): ProdukForm => ({ nama_produk: "", sku: "", harga_jual: "", jumlah_stok: "0", satuan: "pcs", berat_kg: "" });
...
export default function ProdukTab({ C, isDark, showToast }: Props) {
  ...
  const fetchProduk = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from("stok_barang").select("id, nama_produk, sku, jumlah_stok, harga_jual, satuan, berat_kg").order("nama_produk");
  ...
  return (
    <div style={{ animation: "fadeUp 0.3s ease" }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 0.8fr 0.8fr 1fr 0.7fr 0.8fr 150px", gap: 8, padding: "10px 24px", borderBottom: `1px solid ${C.border}`, background: isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)" }}>
            ...
            {editingId !== p.id ? (
              <>
                <div className="data-row" style={{ display: "grid", gridTemplateColumns: "2fr 0.8fr 0.8fr 1fr 0.7fr 0.8fr 150px", gap: 8, padding: "12px 24px", alignItems: "center", transition: "background 0.15s" }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: C.textMid }}>{p.nama_produk}</div>
                  <div style={{ fontSize: 11, color: p.sku ? C.yellow : C.muted, fontFamily: C.fontMono }}>{p.sku || "—"}</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: p.jumlah_stok > 0 ? C.green : C.red, fontFamily: C.fontMono }}>{p.jumlah_stok}</div>
                  ...
                  <div style={{ display: "flex", gap: 5 }}>
                    {confirmDeleteId === p.id ? (
                      ...
                      <>
                        ...
                        <button className="btn-edit" onClick={() => { setEditingId(p.id); setEditForm({ nama_produk: p.nama_produk, sku: p.sku || "", harga_jual: formatIDR(String(p.harga_jual)), jumlah_stok: String(p.jumlah_stok), satuan: p.satuan, berat_kg: p.berat_kg != null ? String(p.berat_kg) : "" }); setShowTambah(false); setKemasanPanel(null); }} style={{ background: `${C.accent}15`, border: `1px solid ${C.accent}30`, color: C.accent, padding: "5px 8px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontFamily: C.fontMono, fontWeight: 700 }}>Edit</button>
      ...
      <div style={{ marginTop: 16, background: `${C.purple}08`, border: `1px solid ${C.purple}20`, borderRadius: 10, padding: "14px 20px", fontSize: 12, color: C.purple, fontFamily: C.fontMono }}>
        📦 <strong>SKU</strong> sesuai kolom SKU Induk di Excel Shopee. &nbsp;·&nbsp; ⚖ <strong>Berat</strong> wajib diisi untuk HPP produksi. &nbsp;·&nbsp; 🎁 <strong>Preset Kemasan</strong> auto pre-fill di form produksi.
      </div>
    </div>
  );
}
