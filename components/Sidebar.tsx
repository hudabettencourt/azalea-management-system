...
export default function Sidebar({ children }: SidebarProps) {
  ...
  useEffect(() => {
    ...
    // onAuthStateChange include INITIAL_SESSION —
    // fired setelah Supabase restore session dari localStorage,
    // jadi lebih reliable dari getSession() untuk first load
  ...
  const SidebarContent = () => (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: "16px 8px" }}>
      <div style={{
        ...
        <button onClick={() => setCollapsed(c => !c)} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: "14px", padding: "4px", borderRadius: "4px", lineHeight: 1 }} title={collapsed ? "Expand sidebar" : "Collapse sidebar"}>
          {collapsed ? "▶" : "◀"}
        </button>
      </div>
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: "12px", marginTop: "8px" }}>
        ...
        <button onClick={handleLogout} title={collapsed ? "Logout" : undefined} style={{ display: "flex", alignItems: "center", justifyContent: collapsed ? "center" : "flex-start", gap: "8px", width: "100%", padding: collapsed ? "10px 0" : "8px 12px", background: "transparent", border: "none", borderRadius: "8px", color: C.muted, fontSize: "13px", fontWeight: 500, cursor: "pointer", transition: "all 0.15s", fontFamily: "'DM Sans', sans-serif" }}
