...
import { NAVIGATION, getActiveModule, getBreadcrumb, type NavModule } from "@/config/navigation";
...
interface AppShellProps {
  ...
  // Action buttons yang muncul di top bar kanan (per halaman)
...
export default function AppShell({ children, actions }: AppShellProps) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_IN" && session?.user)
        ...
        setRole(null); setUserEmail(null); setRoleReady(true);
      }
    });
  ...
  const fetchNotifikasi = useCallback(async () => {
    ...
    (bahanKritis || []).forEach((b: any) => {
      notifs.push({
        ...
        icon: b.stok <= 0 ? <IconAlertCircle size={16} /> : <IconAlertTriangle size={16} />,
    ...
    if (kasData) {
      const saldo = kasData.reduce(
        (s: number, k: any) => k.tipe === "Masuk" ? s + k.nominal : s - k.nominal, 0
      );
      if (saldo < 500000) {
        notifs.push({
          id: "kas-rendah",
  ...
  const SidebarContent = () => (
    <div style={{
      ...
      background: isDark ? C.sidebar : "#ffffff",
    }}>
      {/* Header */}
      <div style={{
        ...
        {(!sidebarCollapsed || isMobile) && (
          <span style={{
            ...
            fontFamily: C.fontSans, letterSpacing: "0.05em",
            textTransform: "uppercase" as const,
          }}>
            {currentMod?.label || "Azalea"}
          </span>
        )}
      </div>
        ...
        {userEmail && (
          <div style={{ marginBottom: 8 }}>
            <div style={{
              ...
              fontFamily: C.fontSans, overflow: "hidden",
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
              <div style={{
                ...
                whiteSpace: "nowrap" as const, maxWidth: 120,
              }}>{userEmail}</div>
              {role && (
                <span style={{
                  background: C.accentGlow, color: C.accent,
                  padding: "1px 6px", borderRadius: 20,
                  fontSize: 9, fontWeight: 800, fontFamily: C.fontSans,
                  flexShrink: 0,
                }}>{role}</span>
              )}
            </div>
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          ...
          {!isMobile && (
            <div style={{
              ...
              background: isDark ? "#0d1812" : "#f8fffe",
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      gap: 3, border: "none", borderRadius: 10,
                      background: active
                        ? (isDark ? "rgba(45,212,191,0.15)" : "rgba(45,212,191,0.12)")
                        : "transparent",
                      color: active ? C.accent : C.muted,
                      cursor: "pointer",
                      transition: "all 0.12s",
                  ...
                  >
                    ...
                    <span style={{
                      fontSize: 9, fontFamily: C.fontSans,
                      fontWeight: active ? 700 : 500,
          ...
          {!isMobile && (
            <div style={{
              width: sidebarCollapsed ? 0 : 210,
              minWidth: sidebarCollapsed ? 0 : 210,
              overflow: "hidden",
              borderRight: sidebarCollapsed ? "none" : `1px solid ${C.sidebarBorder}`,
                ...
                <div style={{
                  ...
                  {visibleModules.map(mod => (
                    <button
                      ...
                      style={{
                        padding: "4px 10px",
                        border: `1px solid ${activeModule === mod.key ? C.accent : C.border}`,
                        borderRadius: 20,
                        background: activeModule === mod.key ? C.accentGlow : "transparent",
                        color: activeModule === mod.key ? C.accent : C.muted,
                        fontSize: 11, fontWeight: activeModule === mod.key ? 700 : 500,
                        cursor: "pointer", fontFamily: C.fontSans,
          ...
          <div style={{
            ...
            <div style={{
              ...
              flexShrink: 0,
              background: isDark ? C.bgNav : "#ffffff",
              ...
              {/* Left: hamburger (mobile) + breadcrumb (desktop) + sidebar toggle */}
...
{!isMobile && (
  <div style={{
    ...
    fontSize: 12, color: C.muted, fontFamily: C.fontSans,
  }}>
    ...
    {breadcrumb.page !== breadcrumb.module && (
      <>
        <IconChevronRight size={12} />
        <span style={{ color: C.text, fontWeight: 600 }}>
          {breadcrumb.page}
              ...
              {/* Right: actions + notif + dark mode + avatar */}
              <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 6 : 8 }}>

                {/* Page actions (per halaman) */}
                {actions && (
                ...
                <div style={{ position: "relative" }} ref={notifRef}>
                  <button
                    ...
                    style={{
                      ...
                      color: C.muted, position: "relative" as const,
                      transition: "all 0.12s",
                    }}
                  >
                    ...
                    {notifList.length > 0 && (
                      <div style={{
                        ...
                        border: `2px solid ${isDark ? C.bgNav : "#ffffff"}`,
                        display: "flex", alignItems: "center", justifyContent: "center",
                  ...
                  {notifOpen && (
                    <>
                      ...
                      <div style={{
                        ...
                        right: isMobile ? -60 : 0,
                        zIndex: 200, width: isMobile ? 300 : 320,
                        background: C.card,
                        ...
                        <div style={{
                          ...
                          <button
                            ...
                            style={{
                              background: "none", border: "none",
                              cursor: "pointer", color: C.muted,
                              display: "flex", borderRadius: 6, padding: 4,
                            }}
                          >
                            <IconRefresh size={14} />
                          </button>
                        ...
                        <div style={{ maxHeight: 360, overflowY: "auto" }}>
                          {notifLoading ? (
                            <div style={{
                              ...
                              color: C.muted, fontSize: 13, fontFamily: C.fontSans,
                            }}>Memuat...</div>
                          ) : notifList.length === 0 ? (
                            <div style={{ padding: 32, textAlign: "center" }}>
                              <div style={{ fontSize: 28, marginBottom: 8 }}>✅</div>
                              <div style={{
                                fontSize: 13, color: C.muted, fontFamily: C.fontSans,
                              }}>Semua aman</div>
                            </div>
                          ) : notifList.map(n => (
                            <a
                            ...
                            >
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{
                                  ...
                                  color: C.text, fontFamily: C.fontSans, marginBottom: 2,
                                }}>{n.title}</div>
                                <div style={{
                                  fontSize: 11, color: C.muted, fontFamily: C.fontMono,
                                }}>{n.desc}</div>
                              </div>
                              <div style={{
                ...
                <div style={{
                  ...
                  background: `linear-gradient(135deg, ${C.accent}, ${C.accentDark})`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: isMobile ? 13 : 12, fontWeight: 800, color: "#fff",
            ...
            <div style={{
              ...
              background: C.bgPage,
              // Mobile bottom padding supaya tidak ketutup bottom nav
        ...
        {!isMobile && (
          <div style={{
            ...
            <div style={{
              ...
              4 toko aktif · AzaleaFood · AsdaFood · ErlinFood · RaizelFood
            </div>
            <div style={{ flex: 1 }} />
            <div style={{
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    // Navigate ke item pertama modul
                    const firstItem = NAVIGATION.find(m => m.key === mod.key)?.groups[0]?.items[0];
                    if (firstItem) router.push(firstItem.href.split("?")[0]);
                  }}
                  style={{
                    display: "flex", flexDirection: "column",
                    alignItems: "center", justifyContent: "center",
                    gap: 3, flex: 1, height: 54,
                    background: "none", border: "none",
                    cursor: "pointer",
                    color: active ? C.accent : C.muted,
                  }}
